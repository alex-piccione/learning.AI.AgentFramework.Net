...
type OrchestratorAgent private (agent:AIAgent, clientWrapper) =
    ...
    static member Create (logger, aiAgentCreator:AIAgentCreator, agentFactory:AgentFactory, clientWrapper:ClientWrapper, rootFolder:string option, ?ct): Task<IChatAgent> = task {
        ...
        let agentDefinition:AgentDefinition = {
            ...
            Instructions = """
                ...
                Your answer is a final result, not an explanation of how you achieve the goal, unless asked by the user.
        ...
        let! fileAgentOpt = task {
            ...
            | None -> return None
            | Some folder -> 
                let! agent = agentFactory.CreateFilesManagerAgent(logger, clientWrapper, folder)
                return Some agent
        }
        ...
        let tools = 
            OrchestratorTools(
                logger,
                agentFactory.CreateWeatherAgent(clientWrapper),
                agentFactory.CreateCryptocurrencyAgent(clientWrapper),
                ?filesAgent=fileAgentOpt
            ).GetTools()

        let! agent = aiAgentCreator.Create(agentDefinition, clientWrapper.ChatClient, tools)
